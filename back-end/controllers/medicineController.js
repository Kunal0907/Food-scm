...
const contractAddress = "0x59679169687B16feF7212B20a0B8584f36120144"; // Update with your deployed contract address
const contract = new web3.eth.Contract(
  MedicineTrackerContract.abi,
  contractAddress
);
...
exports.addMedicine = async (req, res) => {
  try {
    ...
    await contract.methods
      .addMedicine(medicineID, name, manufacturer, expiryTimestamp, count)
      .send({ from: "0x4bbcb8C80E1a0A1Fd0F26dbfeE514bB6Cfd0415C" }); // Update with your wallet address
    ...
    res
      .status(201)
      .json({ success: true, message: "Medicine added successfully" });
  } catch (err) {
    console.error(err);
...
exports.getMedicineById = async (req, res) => {
  try {
    ...
    const expiryDate = new Date(medicine.expiryDate * 1000)
      .toISOString()
      .split("T")[0];
    ...
    res
      .status(200)
      .json({ success: true, data: { ...medicine._doc, expiryDate } });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: "Server Error" });
  }
